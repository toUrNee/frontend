...
const AddFile = ({ prevStep, sitio, imagenes, setImagenes, message }) => {
    ...
    const { getRootProps, getInputProps, isDragActive, isDragAccept, isDragReject } = useDropzone({
        ...
        onDrop: acceptedFiles => {
            ...
            acceptedFiles.forEach(imagen => {
                ...
                axios.post(`${process.env.REACT_APP_BACK_URL}/ArchivoSitioTuristico/CreateArchivoSitioTuristico`, fd)
                    .then(res => {
                        var aux = imagenes
    ...
    const deleteImage = (i) => {
        axios.delete(`${process.env.REACT_APP_BACK_URL}/ArchivoSitioTuristico/DeleteArchivoSitioTuristico`, {
            query: {
                id: imagenes[i]
            }
        })
                ...
                {imagenes.map((id, j) => (
                    <div style={thumb} key={id}>
                        ...
                        <div style={thumbInner}>
                            <img
                                src={`${process.env.REACT_APP_BACK_URL}/ArchivoSitioTuristico/GetArchivoSitioTuristico?id=${id}`}
                                style={img}
                                alt="imagen de sitio"
                            />
                        </div>
